...
internal abstract class DerAdapter<T>(
  ...
  fun readWithTagAndLength(reader: DerReader): T? {
    ...
    if (reader.tag != tag || reader.tagClass != tagClass) {
      throw IOException("expected ${tagClass}/${tag} but was ${reader.tagClass}/${reader.tag}")
    }
  ...
  companion object {
    ...
    fun parseUtcTime(string: String): Long {
    ...
    fun parseGeneralizedTime(string: String): Long {
      val utc = TimeZone.getTimeZone("GMT")
